...
/**
 * Validator for the Oggetto object.
 ...
 */
public class OggettoValidator extends QborrowAbstractValidator<Oggetto> {

    private static Log log = LogFactory.getLog(OggettoValidator.class);

    /**
     * Perform custom validation on a Oggetto object
     * 
     ...
     * @param oggetto object to validate
     * @param groups group name(s) targeted for validation (default to <code>[blank]</code> means all the validation will be done)
     */
    @Override
    protected void customValidation(UserContext userContext, Set<InvalidConstraint<?>> errors, String propertyPath, Oggetto oggetto, String... groups) {
        if (log.isDebugEnabled()) {
            log.debug("Call customValidation method for class OggettoValidator ");
        }

        // insert here custom validations for Oggetto model
