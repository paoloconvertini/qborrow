...
<div class="frameworkRow">
	<div class="frameworkLogoCell">
		<img onclick="document.location.href='<s:property value="#homeUrl"/>';" src="<qs2:imagesPath nome="/logo-qfrmk.png" framework="true" version="2"/>">
	</div>
	<% if("true".equals(request.getParameter("menuActive"))) { %>
		<div class="frameworkHeaderLeftCell">
			<a href="" class="frameworkBottoneMenu" onclick="showMainMenu()" ng-hide="showProjectMenu()" >
	...
	<div class="frameworkHeaderRightCell">
  		...
  		<% if(StringUtils.isNotEmpty((String)ServletActionContext.getRequest().getSession().getAttribute("returnurl"))) { %>
  			<a class="return-button" href="<%=(String)ServletActionContext.getRequest().getSession().getAttribute("returnurl") %>">
   			<span>
   				<%=(String)ServletActionContext.getRequest().getSession().getAttribute("returnbuttonname") %>
   			</span>
  			</a>
  		<% } %>
  		<% if("false".equals(request.getParameter("menuActive"))) { %>
			<a href="" class="frameworkBottoneMenu" onclick="showMainMenu()" ng-hide="showProjectMenu()" >
          	<span class="qsr-only"><s:text name="header.togglenavigation" /></span>
  		</a>
	 <% } %>
		<div class="frameworkHeaderRightCellUserDiv qpull-right" style=" background: url('<qs2:imagesPath nome="/user.jpg" framework="true" version="2"/>') left center no-repeat; background-size: 30px 30px; min-width: 30px;">
			<a href="#" class="qdropdown-toggle user-menu-link" data-toggle="qdropdown">
          	...
          	<ul class="qdropdown-menu user-dropdown-menu">
              	<li class="user-header">
                   	<img src="<qs2:imagesPath nome="/user.jpg" framework="true" version="2"/>" class="qimg-circle" alt="<s:text name="header.userimage"/>" />
