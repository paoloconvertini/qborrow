...
//		this.options.currentFormat = this.options.timePicker ? this.options.dateTimeFormat : this.options.dateFormat;
		...
		if (this.options.icon) {
			// tolto position: relative perch� d� problemi di sovrapposizione in pagina
			//var cont = new Element('div', {'style': 'position: relative;float:left;'});
			var cont = new Element('div', {'style': 'float:left;'});
			...
			Element.setStyle(this.icon, {'vertica-align': 'center'});
		...
		if (this.options.timePicker) {
			...
			otherInput.onkeyup = function(e) {
						if (!isNaN(otherInput.value)) {
							...
							this.dateChanged(this.currentDate, true);
						}
